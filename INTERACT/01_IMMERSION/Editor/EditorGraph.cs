...
namespace InteractEditor.Immersion
{
	/// <summary>
	...
	public class EditorGraph
	{
		...
		public Vector3 UnitToGraph(float x, float y)
		{
		...
		public void DrawWireCircle(Vector2 center, float radius, int width = 1, int nPoints = 30)
		{
			float l_step = 2f * Mathf.PI / nPoints;

			for (int l_i = 1; l_i <= nPoints; l_i++)
			{
				float l_theta = l_step * (l_i - 1);
				float l_x = Mathf.Cos(l_theta) * radius;
				float l_y = Mathf.Sin(l_theta) * radius;

				Vector3 l_p0 = new Vector3(l_x, l_y, 0f);

				l_theta = l_step * l_i;
				l_x = Mathf.Cos(l_theta) * radius;
				l_y = Mathf.Sin(l_theta) * radius;

				Vector3 l_p1 = new Vector3(l_x, l_y, 0f);

				DrawLine(l_p0.x, l_p0.y, l_p1.x, l_p1.y, Handles.color, width);
			}
		}

		public void DrawSolidCircle(Vector2 center, float radius, int width = 1, int nPoints = 30)
		{
			List<Vector3> l_points = new List<Vector3>(nPoints);
			float l_step = 2f * Mathf.PI / nPoints;

			for (int l_i = 0; l_i < nPoints; l_i++)
			{
				float l_theta = l_step * (l_i - 1);
				float l_x = Mathf.Cos(l_theta) * radius;
				float l_y = Mathf.Sin(l_theta) * radius;

				Vector3 l_p = new Vector3(UnitToGraphX(l_x), UnitToGraphY(l_y), 0f);

				l_points.Add(l_p);
			}

			Handles.DrawAAConvexPolygon(l_points.ToArray());
		}

		public void DrawSolidArc(Vector3 center, Vector3 from, float angleDeg, int nPoints = 30)
		{
			List<Vector3> l_points = new List<Vector3>(nPoints);
			Quaternion l_angleStep = Quaternion.Euler(new Vector3(0f, 0f, angleDeg / nPoints));

			Vector3 begin = Quaternion.Euler(0, 0, -(angleDeg / 2f)) * from;

			for (int i = 0; i <= nPoints; i++)
			{
				l_points.Add(new Vector3(UnitToGraphX(begin.x), UnitToGraphY(begin.y), 0));
				begin = l_angleStep * begin;
			}

			l_points.Add(new Vector3(UnitToGraphX(center.x), UnitToGraphY(center.y), 0));
