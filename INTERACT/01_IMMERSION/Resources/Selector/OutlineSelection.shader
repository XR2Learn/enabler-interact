...
#include "UnityCG.cginc"
		...
		fixed4 NullPs(PS_INPUT i) : SV_Target
	{
		...
		v2f z;
		z.pos = UnityObjectToClipPos(v.vertex);
		z.uv.xy = TRANSFORM_TEX(v.texcoord, _MainTex);
		z.vertexColor = v.color * _MainColor;
		return z;
	}
	...
	fixed4 frag(v2f i) : COLOR{
		...
		SubShader
		{
			//la queue doit etre a transparent +x pour l'interieur'
